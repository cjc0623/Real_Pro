...
const MemberOwner = () => {
  ...
  const handleOpenModal = (user) => {
    setSelectedUserForModal({ ...user, userType: 'OWNER' });
    setOpenModal(true);
  };
          ...
          <Box sx={{ display: { xs: 'block', md: 'none' } }}>
            {rows.length > 0 ? (
              rows.map((r, i) => (
                ...
                >
                  <Box display="flex" justifyContent="space-between" alignItems="baseline" mb={1.5}>
                    ...
                    <Typography variant="caption" color="#94a3b8" fontWeight="600">{fmt(r.memCreateidDateTime)}</Typography>
                  </Box>
                  <Stack spacing={0.8}>
              ...
              <Paper elevation={0} sx={{ p: 5, textAlign: 'center', borderRadius: "20px", border: "1px solid #f1f5f9" }}>
                <Typography color="#94a3b8" fontWeight="500">가입된 물주 회원 데이터가 존재하지 않습니다.</Typography>
              </Paper>
            )}
          </Box>
              ...
              <TableBody>
                {rows.map((r, i) => (
                  <TableRow key={r.memId ?? i} onClick={() => handleOpenModal(r)} style={{ cursor: 'pointer' }} sx={{ '&:hover': { bgcolor: '#f8fafc' } }}>
                    ...
                    <TableCell sx={{ color: '#64748b' }}>{fmt(r.memCreateidDateTime)}</TableCell>
                  </TableRow>
                ))}
                {rows.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={4} align="center" sx={{ py: 6, color: '#94a3b8', fontWeight: 500 }}>
                      가입된 물주 회원 데이터가 존재하지 않습니다.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
